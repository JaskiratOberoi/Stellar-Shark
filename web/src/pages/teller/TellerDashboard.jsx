...
/** Typography helpers (Unicode escapes keep this file safe if the editor saves as non-UTF-8). */
const MID = '\u00b7';
const EM = '\u2014';
const ELL = '\u2026';
const ARR = '\u2192';
...
export function TellerDashboard() {
    ...
    const handleShare = useCallback(async () => {
        const lines = [
            `Nexus by Stellar Infomatica ${EM} Daily test volume`,
            `Total samples: ${totalDisplay ?? EM}`,
            `Unique SIDs: ${sidsDisplay ?? EM}`,
            `Assigned BU: ${assignedBu}`,
            `Test code: ${(result?.testCode ?? testCode).trim() || EM}`,
            `Range: ${dateFrom} ${ARR} ${dateTo}`
        ];
        try {
            ...
            <div className="relative z-10 flex flex-1 min-h-0 min-w-0 flex-col md:flex-row">
                <aside className="w-full md:w-[320px] md:shrink-0 border-b md:border-b-0 md:border-r border-ink bg-surface flex flex-col max-h-[55dvh] md:max-h-none overflow-y-auto log-scroll">
                    ...
                    <div className="p-5 space-y-6 flex-1 flex flex-col">
                        <section>
                            <h2 className="text-[10px] font-semibold uppercase tracking-wider text-ink-muted mb-3">
                                Timeframe
                            </h2>
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="text-[10px] text-ink-muted mb-1 block" htmlFor="date-from">
                                        From
                                    </label>
                                    <input
                                        id="date-from"
                                ...
                                <div>
                                    <label className="text-[10px] text-ink-muted mb-1 block" htmlFor="date-to">
                                        To
                                    </label>
                                    <input
                                        id="date-to"
                            ...
                            {rangeInvalid && (
                                <p className="text-danger text-[10px] mt-2">
                                    From must be {LEQ} To.
                                </p>
                            )}
                            <div className="flex flex-wrap gap-1.5 mt-3">
                                {primaryQuick.map(({ id, label }) => (
                                    <button
                                        ...
                                        className="px-2.5 py-1.5 rounded-lg border border-border text-[10px] font-bold uppercase tracking-wider text-ink-secondary hover:bg-surface-muted disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
                                    >
                                        {label}
                                    </button>
                                ))}
                            ...
                            <div className="flex flex-wrap gap-1 mt-1.5">
                                {secondaryQuick.map(({ id, label }) => (
                                    <button
                                        ...
                                        disabled={running}
                                        onClick={() => applyPreset(id)}
                                        className="px-2 py-1 rounded text-[9px] font-semibold uppercase tracking-wide text-ink-muted hover:text-ink-secondary hover:bg-surface-muted disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
                                    >
                                        {label}
                                    </button>
                                ))}
                        ...
                        <div className="mt-auto pt-2 space-y-2">
                            ...
                            >
                                <Play className="w-5 h-5 shrink-0" strokeWidth={2.25} aria-hidden />
                                {running ? `Running analysis${ELL}` : 'Run New Analysis'}
                            </motion.button>
                            {running ? (
                                <button
                                    type="button"
                                    onClick={stop}
                                    className="w-full text-center text-[10px] uppercase tracking-wider text-ink-muted hover:text-ink-secondary py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30 rounded"
                                >
                                    Cancel
                                </button>
                            ) : null}

                            <details className="text-[10px] text-ink-faint pt-2 border-t border-border group">
                                <summary className="cursor-pointer text-ink-muted hover:text-ink-secondary list-none flex items-center gap-1 [&::-webkit-details-marker]:hidden">
                                    <ChevronRight
                                ...
                                <label className="flex items-center gap-2 cursor-pointer mt-2 text-ink-muted">
                                    <input
                                        type="checkbox"
                                        checked={!headless}
                                ...
                                <p className="mt-2 leading-relaxed pl-1 border-l border-border">
                                    <code className="text-primary">LIS_*</code> or{' '}
                                    <code className="text-primary">CBC_LOGIN_*</code> in{' '}
                        ...
                        <>
                            <header className="flex flex-wrap items-end justify-between gap-4 mb-6 pb-4 border-b border-ink">
                                ...
                                <div className="flex items-center gap-3">
                                    {running ? (
                                        <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary/30 bg-primary-soft text-[10px] font-bold uppercase tracking-widest text-primary">
                            ...
                            {error ? (
                                <p className="mb-4 text-sm text-danger border border-danger/30 rounded-xl px-3 py-2 bg-danger-soft">
                                    {error}
                                </p>
                            ) : null}
                            ...
                            <div className="flex-1 min-h-0 overflow-y-auto log-scroll space-y-4 pr-1">
                                ...
                                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 pb-4">
                                    ...
                                    <div className="lab-card">
                                        <h3 className="text-sm font-semibold text-ink mb-3">Per business unit</h3>
                                        <BuSummaryTable
                                            rows={buSummaryRows}
                                            variant="lab"
                                            aggregate={aggregate}
                                        ...
                                        <details className="mt-4 group">
                                            <summary className="text-[10px] font-semibold uppercase tracking-wider text-ink-muted cursor-pointer list-none flex items-center gap-1 [&::-webkit-details-marker]:hidden">
                                                <ChevronRight
